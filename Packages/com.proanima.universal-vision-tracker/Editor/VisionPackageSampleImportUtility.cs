...
{
    internal static class VisionPackageSampleImportUtility
    {
        public const string PackageJsonPath =
        ...
        public static bool TryImport(string sampleDisplayName, out string error)
        {
            error = null;
            ...
            object sample = FindSampleByDisplayName(sampleType, samples, sampleDisplayName);
            if (sample == null)
            {
                error = $"{sampleDisplayName} sample was not found in the package manifest.";
                return false;
            }

            return TryImportSample(sampleType, sample, out error);
        }

        public static void ShowImportHelp(string title, string sampleDisplayName, string importError)
        {
            bool packageManagerOpened = TryOpenPackageManager();
            string message = packageManagerOpened
                ? $"In Package Manager, select ProAnima Universal Vision Tracker, open Samples, and import {sampleDisplayName}."
                : $"Open Window > Package Management > Package Manager, select ProAnima Universal Vision Tracker, open Samples, and import {sampleDisplayName}.";
